...
**v0.11 也要順手改一組:** `docker-compose.yml` 裡 `food_order_minio` 服務的
`MINIO_ROOT_USER`/`MINIO_ROOT_PASSWORD`,以及 `food_order_api` 服務裡對應的
`MINIO_ACCESS_KEY`/`MINIO_SECRET_KEY` -- 這四個值兩兩要一致(帳號對帳號、密碼對
密碼),預設的 `CHANGE_ME_MINIO_PW` 一樣是佔位用的,部署前記得換掉。
...
Windows11上傳
```bash
# 1. 切換回 main,並確保同步雲端最新的程式碼
git checkout main
git pull origin main

# 改動程式碼後,提交並推送(建議都直接在 main 上做,不要再切 feature 分支,比較不會搞混):
git add -A
...
# 2. 更新env(第一次部署才需要;已經有 .env 的話跳過,直接編輯裡面缺的欄位就好)
cp .env.example .env
nano .env   # 填入 GEMINI_API_KEY=...(Gemini 2.0 Flash 已停用,改用 2.5)/OPENAI_API_KEY=... 等
...
### 之後接 Tailscale(或任何其他網路)

v0.10 之後,前端**不再**把後端網址(`VITE_API_BASE`)烤進靜態檔案裡了——之前那種做法只認
build time 寫死的那一個 LAN IP,換一個網路(例如手機用 Tailscale 連進來,拿到的是
`100.x.x.x` 這種 Tailscale IP,不是內網 IP)就會整個連不上,編輯餐廳存檔一直跳「連不上
伺服器」就是這個原因。

現在 `frontend/nginx.conf` 會把 `/api/*` 的請求原地轉發給 `food_order_api` 容器(在 Docker
內部網路裡轉,瀏覽器不會知道),前端呼叫 API 一律用相對路徑,等於「不管你用哪個網址/哪個
網路連進 `food_order_web`,API 都會自動打回同一台伺服器」。這代表:

- 區網 IP、Tailscale IP、`mike1-virtual-machine` 這種內部主機名稱,同一份 build 全部都能用,
  **不需要因為換網路就重新 build 前端**。
- 裝 Tailscale 只要確保手機能連到這台 VM 的 `55173` port(用 VM 的 Tailscale IP 或
  MagicDNS 名稱都行,例如 `http://100.x.x.x:55173` 或 `http://mike1-vm.tailxxxx.ts.net:55173`),
  就能直接用,不用再改任何設定檔。

### 餐廳照片改存物件儲存(MinIO,v0.11)

v0.11 之前,上傳的每張餐廳照片都是把整張圖轉成 `data:` base64 字串,直接塞進 Postgres 的
`TEXT` 欄位——簡單但不 scale,圖片一多資料庫(和每一次備份)都會跟著變肥。現在改用自架的
MinIO(S3-compatible 物件儲存,`docker-compose.yml` 新增的 `food_order_minio` 服務)存放實際
圖片,資料庫只存一個短路徑(例如 `/media/5/ab12cd.jpg`)。

**這是全自動的,不需要你手動設定金鑰**(跟 Gemini/OpenAI/Google Places 那幾個不一樣)——
`food_order_minio` 的帳號密碼直接寫在 `docker-compose.yml` 裡(記得照上面步驟 3 的提醒改掉
預設密碼),`food_order_api` 啟動時會自動建立 bucket、設成匿名可讀,並且**自動把資料庫裡舊的
base64 照片搬過去**(只搬一次,搬過的不會重複搬,不影響既有照片)。

如果想確認搬移結果或手動檢查儲存空間,可以打開 MinIO 的管理主控台:

```
http://<VM的IP或Tailscale位址>:59001
```

用 `docker-compose.yml` 裡的 `MINIO_ROOT_USER`/`MINIO_ROOT_PASSWORD` 登入,就能看到
`restaurant-photos` 這個 bucket 裡實際存的圖片檔案。
...
- 餐廳照片已改成物件儲存(MinIO,v0.11,見上方對應章節),不再整張塞進資料庫。
