"""AI helpers: menu photo parsing (v0.9) + item categorization (v0.10).

Two independent features share the same Gemini-first/OpenAI-fallback
plumbing:

- parse_menu_photo(image_data_url): sends a photo of a menu to a
  vision-capable LLM, gets back a structured list of items (name, price,
  option groups) matching schemas.MenuItemIn.
- classify_menu_categories(item_names, reference_pairs): text-only (no
  image) -- given the current restaurant's item names plus a sample of
  already-categorized (name, category) pairs from elsewhere in the
  database, asks the model to suggest a category per item, preferring to
  reuse an existing category name over inventing a new one.
